...
detect_distribution_version() {
    ...
    if [ -f /etc/os-release ]; then
        ...
        case "$ID" in
            ubuntu)
                sudo wget https://repo.zabbix.com/zabbix/6.0/ubuntu/pool/main/z/zabbix-release/zabbix-release_6.0-4+ubuntu$(grep "DISTRIB_RELEASE" /etc/lsb-release | cut -d'=' -f2)_all.deb \ #rw
